...
def test_calculo_utilizacion(db_connection):
    with db_connection.cursor() as cur:
        ...
        # 2. Alta Producto y Pieza (Area 2500 = 25% de 10000)
        cur.execute("CALL sp_alta_producto('PROD-X', 'Mesa', 1, 'rectangulo', '{\"w\":50, \"h\":50}', 2500);")
        
        # Asignar la pieza a la materia prima manualmente para probar el cálculo
        cur.execute("UPDATE piezas SET materia_prima_id = %s WHERE nombre_pieza = 'Pieza Base PROD-X'", (mp_id,))
...
def test_trigger_eventos_json(db_connection):
    with db_connection.cursor() as cur:
        ...
        # Insertar evento JSON para rotar la pieza a 90 grados en posicion 10,10
        payload = json.dumps({"pieza_id": pieza_id, "x": 10, "y": 10, "rotacion": 90})
        ...
        assert float(data[1]) == 10.00 # Posición X actualizada
